    ...
    <main className="mx-auto max-w-3xl px-4 py-8">
      ...
      <div className="mt-3 flex items-center justify-between gap-4">
        ...
        <div className="text-right">
          ...
          <p className={`text-lg font-semibold ${balanceColor}`}>{money(Math.abs(member.balance))}</p>
        </div>
      </div>
      ...
      <div className="mt-6 flex flex-col gap-3">
        ...
        {member.bills.length === 0 && (
          <p className="text-sm text-slate-500">Not assigned to anything yet.</p>
        )}
          ...
          >
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                {bill.bill_date && <p className="mt-0.5 text-xs text-slate-500">{bill.bill_date}</p>}
              </div>
