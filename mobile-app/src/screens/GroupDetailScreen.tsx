...
export default function GroupDetailScreen({ route, navigation }: Props) {
  ...
  function handleDeleteGroup() {
    Alert.alert(
      'Delete group?',
      `Delete "${group?.name}"? This removes all its bills and settlements.`,
      [
  ...
  function handleRemoveMember(member: GroupMember) {
    Alert.alert('Remove buddy?', `Remove ${member.name} from this group?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Remove',
        style: 'destructive',
        onPress: () => removeMember({ groupId, memberId: member.id }),
      },
    ]);
  }
        ...
        <View className="flex-row rounded-full bg-gray-100 p-1">
          <TouchableOpacity
            ...
            className={`rounded-full px-5 py-1.5 ${
              tab === 'buddies' ? 'bg-white' : ''
            }`}
          >
            <Text
              className={`text-sm font-medium ${
                tab === 'buddies' ? 'text-gray-900' : 'text-gray-400'
              }`}
            >
              Buddies
            </Text>
          ...
          >
            <Text
              className={`text-sm font-medium ${
                tab === 'bills' ? 'text-gray-900' : 'text-gray-400'
              }`}
            >
              Bills
            </Text>
...
function BillsPanel({
  ...
  return (
    <View className="gap-2">
      {bills?.length === 0 && (
        <Text className="text-sm text-gray-500">No bil in this group yet.</Text>
      )}
      {bills?.map(bill => (
        <TouchableOpacity
          key={bill.id}
...
function BuddiesPanel({
  ...
  const { data: searchResults } = useSearchUsersQuery(query.trim(), {
    ...
    () =>
      (searchResults ?? []).filter(
        u =>
          ...
          !matchingBuddies.some(b => b.buddy_user_id === u.id),
      ),
      ...
      {query.trim().length > 0 && (
        <View className="mt-2 rounded-xl border border-gray-100 bg-white py-1">
          {matchingBuddies.length > 0 && (
            <>
              <Text className="px-3 pt-1 text-[11px] font-medium uppercase tracking-wide text-gray-400">
                Your buddies
              </Text>
              {matchingBuddies.map(b => (
                <TouchableOpacity
                  key={b.buddy_user_id}
                  onPress={() => addByUser(b.buddy_user_id, b.user.name)}
                  className="px-3 py-2"
                >
      ...
      <View className="flex-row items-start justify-between">
        <View className="flex-1">
          ...
          <Text className="mt-1 text-xs text-gray-500">
            Everyone in this group pays their share to whoever is set here.
          </Text>
        </View>
      ...
      {payer ? (
        <View className="mt-3 rounded-lg bg-gray-50 px-3 py-2">
          ...
          {payer.user?.bank_name || payer.user?.bank_account_number ? (
            <Text className="mt-1 text-xs text-gray-500">
              {payer.user.bank_name}{' '}
              {payer.user.bank_account_number
                ? `— ${payer.user.bank_account_number}`
                : ''}
            </Text>
          ) : (
        ...
        <Text className="mt-3 text-sm text-gray-500">
          {isCreator
            ? 'No payer set yet — tap the pencil to choose one.'
            : 'No payer has been set for this group yet.'}
        </Text>
      )}
...
function EditPayerModal({
  ...
  async function handleSave() {
    await updateGroup({
      groupId,
      payer_id: selected === 0 ? null : selected,
    }).unwrap();
    onClose();
  }

  const options = [
    { label: 'No payer set', value: 0 },
...
function BalancesSection({
  ...
  const [createSettlement, { isLoading: isPaying }] =
    useCreateSettlementMutation();
      ...
      <View className="mt-3 gap-2">
        {balances.map(b => {
          const canMarkPaid =
            ...
            b.status === 'pending' &&
            payerId &&
            (b.group_member_id === myMemberId || myMemberId === payerId);
          ...
          return (
            ...
            >
              ...
              <Text className="text-sm text-gray-500">
                {money(Math.abs(b.gross_balance))}
              </Text>
            </View>
          );
        })}
          ...
          <View className="mt-2 gap-1.5">
            {settlements.slice(0, 5).map(s => (
              <Text key={s.id} className="text-xs text-gray-600">
                {s.payer?.name ?? 'Someone'} paid {s.payee?.name ?? 'someone'}{' '}
                {money(s.amount)}
              </Text>
            ))}
          </View>
